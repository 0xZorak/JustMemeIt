...
router.get('/callback', async (req, res) => {
  ...
  try {
    ...
    res.redirect(`http://localhost:3000/x-auth-success?name=${encodeURIComponent(userRes.data.data.name)}&profile_image_url=${encodeURIComponent(userRes.data.data.profile_image_url)}`);
  } catch (err) {
